...
pub fn funnel(chain: &ChainSnapshot, policy: &Policy) -> Top20 {
    ...
    for c in chain.puts() {
        ...
        // Layer 6 SVI skipped (flag off). Utility = efficiency |Δ|/mid.
...
mod tests {
    ...
    fn fixture_funnel_has_puts_excludes_short_dte() {
        ...
        assert!(top.contains_occ(pick.occ_symbol.as_str()));
    }

    #[test]
    fn hold_and_breach_on_dollar_delta() {
        assert_eq!(
            band_status(dollar_delta_stock(1.0, 500.0), -10_000.0, 10_000.0),
            BandStatus::Hold
        );
        assert_eq!(
            band_status(dollar_delta_stock(1_000.0, 500.0), -10_000.0, 10_000.0),
            BandStatus::Breach
        );
    }
